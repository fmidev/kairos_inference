...
def read_grib_time(gribfile, read_coordinates=False):
            ...
            if read_coordinates and len(lons) == 0:
                ...
                for j in range(nj):
                    ...
                    for i in range(ni):
                        ...
                        lons.append(lon)
                        lats.append(lat)
        # Order according to the forecasttime
        # Convert to a single NumPy array: shape = (n_steps, nj, ni)
        values = np.asarray(values)
        # Get the indices that would sort forecasttime ascending
        order = np.argsort(forecasttime)
        # Reorder the Python list of datetimes
        forecasttime = [forecasttime[i] for i in order]
        # Reorder the NumPy array along its first axis
        values = values[order, ...] 
        #print(forecasttime)
