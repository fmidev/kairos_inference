...
def ml_forecast_vis(df, vis_models):
    """
    ...
    if len(vis_models) == 2:
        start = [3, 6, 12] # for 2 models
    elif len(vis_models) == 3:
        start = [3, 6, 9, 12] # for 3 models
    for i in range(len(vis_models)):
        # Load the model
        startd = start[i]
        endd = start[i+1]
...
def nearest_idx(nearest):    
    col1 = math.floor(nearest/949) # MEPS domain is 949×1069 points
    row1 = round(nearest - col1*949)
    idxs = np.array([
        [col1, row1],
        [col1, row1+1],
        [col1+1, row1],
...
def main():
    ...
    # Obs lat/lon
    # Efro: 66.56236  25.83814
    icao = ['EFHK','EFRO']  # Helsinki and Rovaniemi
    nearest_1 = pd.DataFrame({"EFHK":[396376],"EFRO":[659221]})
    latlon = pd.DataFrame(   {
            "lat": [60.329368, 66.56236],
            "lon": [24.97274, 25.83814]
        }
    )
...
#    ], dtype=np.int32)
    ...
    # Create gridpp location classes 
    grid = gridpp.Grid(lats,lons)
    # make template grid for the output data
    ...
    for i in range(len(icao)):
        print(f"Processing airport {icao[i]} at lat/lon: {latlon['lat'][i]}/{latlon['lon'][i]}")
        points = gridpp.Points([latlon["lat"].iloc[i]], [latlon["lon"].iloc[i]])
        #idxs = grid.get_closest_neighbours(float(latlon.loc[0, "lat"]),float(latlon.loc[0, "lon"]), 4)
        nearest = nearest_1[icao[i]].values[0]
        idxs = nearest_idx(nearest)
        print("Nearest grid points indices:", idxs)
        ...
        latest_metar = latest_metar_observations_features(icao[i], atimestr)
        # Combine with MEPS data
        ...
        if args.parameter == "visibility":
            # read in ml models
            if icao[i] == 'EFHK':
                model_vis_0 = f"xgb_vis_{icao[i]}_0_1025.json" # EFHK: 1-3h 
                model_vis_1 = f"xgb_vis_{icao[i]}_1_1025.json" # EFHK: 4-9h
                vis_models = [model_vis_0, model_vis_1]
            elif icao[i] == 'EFRO':
                model_vis_0 = f"xgb_vis_{icao[i]}_0_1025.json" # EFRO: 1-3h 
                model_vis_1 = f"xgb_vis_{icao[i]}_1_1025.json" # EFRO: 4-6h
                model_vis_2 = f"xgb_vis_{icao[i]}_2_1025.json" # EFRO: 7-9h
                vis_models = [model_vis_0, model_vis_1, model_vis_2]
    ...
    lon_min, lon_max = 23, 26 #lons.min(), lons.max()
    lat_min, lat_max = 59, 68 #lats.min(), lats.max()
